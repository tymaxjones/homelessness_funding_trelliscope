...
First we load in our data and choose our categorical columns in the order that we want them to appear in our sankey charts.
...
We want our trelliscope to wrap based on state so we will nest our dataframe based on the `state_company` variable.

```{r message=FALSE, warning=FALSE}
dat <- nest(dat, -"state_company")
```

Now we create a function that can be applied to each state that will create a sankey chart.
...
With out sankey chart function we will apply it to every states dataframe
